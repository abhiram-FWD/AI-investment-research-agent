...
export const ThesisSchema = z.object({
  ...
  keyRisks: z
    .array(z.string())
    .describe(
      "Specific, named risks (regulatory, competitive, execution, etc.)",
    ),
  summary: z
    .string()
    .describe(
      "2-3 sentence plain-language summary of the company's current position",
    ),
  newsSentiment: z
    .array(
      z.object({
        title: z
          .string()
          .describe("Must exactly match a news article title from the input"),
        sentiment: z.enum(["positive", "negative", "neutral"]),
      }),
    )
    .describe(
      "One sentiment classification per news article provided in RECENT NEWS, matched by exact title",
    ),
});
...
export const VerdictSchema = z.object({
  ...
  confidence: z
    ...
    .min(0)
    .max(1)
    .describe("0-1 confidence in this verdict"),
  reasoning: z
    .string()
    .describe("Why this verdict follows from the scoring rubric"),
